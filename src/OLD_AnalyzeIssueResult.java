...
public class OLD_AnalyzeIssueResult {
            ...
            try {
                ...
                if (RepoID != -1) {
                    ...
                    if (f.exists()) {
                        ...
                        String query = "  INSERT INTO fork.old_ISSUE (issue_id, author, closed, created_at, updated_at, closed_at, title,projectID) " +

                                " SELECT * FROM (SELECT ? AS a,? AS b,? AS c,? AS d,? AS e,? AS f, ? AS ds,? AS de) AS tmp" +
                                " WHERE NOT EXISTS (" +
                                " SELECT * FROM  fork.old_ISSUE WHERE projectID = ? AND issue_id = ?" +
                                ") LIMIT 1";
...
//                                        "VALUES (?,?,?,?,?,?,?,?)";
    ...
    private static void updatePr_issue_table(String repo) {
        ...
        Connection conn;
        String query = "  INSERT INTO fork.old_PR_TO_ISSUE (repoID, pull_request_id, issue_id) " +
//                                                "VALUES (?,?,?)";
                " SELECT * FROM (SELECT ? AS a,? AS b,? AS c) AS tmp" +
                " WHERE NOT EXISTS (" +
                "SELECT * FROM fork.old_PR_TO_ISSUE WHERE repoID = ? AND pull_request_id=? AND issue_id = ?" +
                ") LIMIT 1";
        try {
