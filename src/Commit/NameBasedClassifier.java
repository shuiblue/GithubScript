...
//        tmpDirPath = current_dir + "/cloneRepos/";
        ...
        if (current_OS.indexOf("mac") >= 0) {
            ...
            result_dir = "/Users/shuruiz/Box Sync/ForkData/result/";

        } else {
            tmpDirPath = "/home/feature/shuruiz/ForkData/cloneRepos/";
            result_dir = "/home/feature/shuruiz/ForkData/result/";
    ...
    public HashMap<String, ArrayList<String>> getForkIdMap(String repo_url) {
        ...
        for (String fork_contact_info : forkowner_Array) {
            if (!fork_contact_info.equals("")) {
                ...
                String[] emailarray = io.removeBrackets(fork_contact_info.split(",\\[")[1]).split(",");
...
//                HashSet<Commit.CommitType> commitType_set = checkCommitType(commit, fork_url, email_list, parent_id);
...
//    #original commits, #sync_upstream commits, #branch_merge commits, #Pull_Request.PullRequest commits, #other, " +
//    "commits only in fork, commits only in upstream,merged commits\n");
    ...
    public ForkInfo getParentInfo(String parent_sha_2, String repo_url) {
...
//        String[] issue_PR = {"issue", "pr"};
    ...
    private void analyzeContribution(String upstream_url) {
        ...
        for (int i = 1; i < commits_in_fork.length; i++) {
            List<String> commits_only_in_fork = Arrays.asList(io.removeBrackets(commits_in_fork[i].split(",")[5].split(",")[0]).split("/ "));
            ...
            if (pr_array.length > 1) {
                for (int j = 1; j < pr_array.length; j++) {
                    ...
                    if (!pr_state.equals("merged")) {
                        String[] rejected_commits = io.removeBrackets(pr_info[1]).split(", ");
